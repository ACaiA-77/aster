"""Read-only helpers for Aster run/session evidence.

These helpers intentionally do not import or instantiate the Aster runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.aster/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".aster" / "runs")
        ...
        session_path = _latest_file(workspace / ".aster" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".aster" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
