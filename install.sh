...
REPO="${ASTER_REPO:-https://github.com/martin-los/aster.git}"
INSTALL_DIR="${ASTER_INSTALL_DIR:-$HOME/.aster-agent}"
BRANCH="${ASTER_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[aster]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[aster]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[aster]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[aster] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    BIN_DIR="${ASTER_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/aster"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/aster" "\$@"
EOF
    chmod +x "$LAUNCHER"
