...
aster 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
保留运行证据，并把有价值的上下文沉淀成本地记忆。

</div>

<p align="center">
  <img src="assets/screenshots/aster-tui-intro-v2.png" alt="aster TUI 启动界面" width="960">
</p>
...
aster 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
- **session / run evidence**：对话、事件流、trace、report 都写到本地 `.aster/`。
- **memory / dream**：把 daily log 整理成长期 topic，下次 session 可以继续用。

aster 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
| --- | --- |
| ![aster TUI 工具表](assets/screenshots/aster-tui-tools-v2.png) | ![aster TUI skills 和 help](assets/screenshots/aster-tui-skills-help-v2.png) |

| Memory 和 durable topics | Slash command 工作区 |
| --- | --- |
| ![aster TUI memory 和 skills](assets/screenshots/aster-tui-memory-skills-v2.png) | ![aster TUI slash command 补全](assets/screenshots/aster-tui-latest-v2.png) |
...
```bash
curl -fsSL https://raw.githubusercontent.com/martin-los/aster/main/install.sh | bash
```

源码安装：

```bash
git clone https://github.com/martin-los/aster.git
cd aster
pip install -e .
```
...
aster 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
...
```text
CLI 参数 > 环境变量 > 项目 .aster.toml > 全局 ~/.config/aster/config.toml > 代码默认值
```
...
```bash
cp .aster.toml.example .aster.toml
$EDITOR .aster.toml
```

`.aster.toml` 默认被 `.gitignore` 忽略，不要把真实 key 提交进 git。
...
```bash
export ASTER_PROVIDER=deepseek
...
export ASTER_BASE_URL=https://api.openai.com/v1
export ASTER_MODEL=gpt-5.4
```
...
```bash
aster --provider openai --model gpt-5.4 --base-url https://api.openai.com/v1
aster --provider deepseek --approval ask --max-steps 80
aster --config /path/to/custom.toml --cwd /path/to/repo
```
...
```bash
aster                              # 默认 Textual TUI
aster --repl                       # 普通终端 REPL
aster "找出测试失败的根因"          # one-shot 任务
aster --resume latest              # 续接最近 session
aster --cwd /path/to/repo          # 指定工作目录
```

常用运行参数：

```bash
aster --approval ask               # shell / 写文件前询问
aster --approval auto              # 普通操作自动通过
aster --approval never             # 非交互模式
aster --sandbox best_effort        # 尽量隔离 shell 命令
aster --no-auto-dream              # 关闭后台 memory 整合
```
...
| --- | --- |
| 项目配置 | `.aster.toml` |
| 全局配置 | `~/.config/aster/config.toml` |
| 会话历史 | `.aster/sessions/<id>.json` |
| 事件流 | `.aster/sessions/<id>.events.jsonl` |
| 运行证据 | `.aster/runs/<run_id>/` |
| 记忆索引 | `.aster/memory/MEMORY.md` |
| Daily logs | `.aster/memory/logs/YYYY/MM/YYYY-MM-DD.md` |
| Durable topics | `.aster/memory/topics/*.md` |
| 用户 skills | `~/.aster/skills/<name>/SKILL.md` |
| 项目 skills | `skills/<name>/SKILL.md` 或 `.aster/skills/<name>/SKILL.md` |
...
ASTER_LIVE_SMOKE=1 pytest tests/test_release_smoke.py -q
```
...
| 入口 | 内容 |
| --- | --- |
| [配置](docs/configuration.md) | provider profile、`.aster.toml`、环境变量和 sandbox 配置。 |
