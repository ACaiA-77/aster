# Aster v3 review pack

## Project pitch

Aster is a small local coding agent that turns one user request into a bounded
...
- `Aster` owns session state, memory, tools, and workspace safety.
...
be validated through persisted `.aster/sessions/*.json` and
`.aster/sessions/*.events.jsonl` artifacts before treating a runtime change as
done.
...
- `.aster/runs/<run_id>/task_state.json`
- `.aster/runs/<run_id>/trace.jsonl`
- `.aster/runs/<run_id>/report.json`
